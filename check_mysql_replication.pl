...
# Changelog:
#  20101104 - Fix port handling, and use of a default database for connect.
#	      This one may be worth upgrading to; bump version to 0.06.
#  20101103 - Clean up some subs to make them a little more compact
#           - Clean up version and help options ot show just script name 
#             (ignoring path if it was used for $0)
#           - Include subversion commit ID in the script
...
my ($subversion_revid) = q$Revision$ =~ /(\d+)/;
...
my $options = {
    ...
    'slave-pass'                    => 'password',
    'debug'                         => 0,
    'table-rows-diff-absolute-crit' => 10,
    'table-rows-diff-absolute-warn' => 5
};
GetOptions(
    $options,                "slave=s",
    "slave-user=s",          "slave-pass=s",
    "master=s",              "master-port=i",
    "master-user=s",         "master-pass=s",
    "crit=s",                "warn=s",
    "help",                  "slave-port=i",
    "schema=s",
    "debug=i",               "version",
    "check-random-database", "table-rows-diff-absolute-crit=i",
    "table-rows-diff-absolute-warn=i"
);
my $max_binlog;

if ( defined $options->{'help'} ) {
    print <<FOO;
$progname: check replication between MySQL database instances
...
FOO
    ...
    printf "%s %s (svn %s)\n", $progname, $VERSION, $subversion_revid|| "-";
    exit;
}
...
sub get_status {
    ...
    debug( 1,
        "Connecting to slave $host:$port as user " . $options->{'slave-user'} );
    my $dbh = DBI->connect(
        "DBI:mysql:database=$schema;host=$host;port=$port",
        $options->{'slave-user'},
        $options->{'slave-pass'}
    );
    if ( not $dbh ) {
        print "UNKNOWN: Cannot connect to slave $host:$port\n";
        exit 3;
    }
    ...
    $res = $sth->execute();
    if (not defined ($res) || $res !~ /^\d+$/) {
      debug(1, "Slave data fetch failed");
      print "UNKNOWN: Cannot connect to slave\n";
      exit 3;
    }
    $slave_data->{replication} = $sth->fetchrow_hashref;
    $sth->finish;

    debug(4, Dumper($slave_data));

    if ( defined $options->{'check-random-database'} ) {
        debug( 2, "Checking status of a random replicate-do-db" );
        my @replicated =
          split( ',', $slave_data->{replication}->{Replicate_Do_DB} );
        my $random_db = $replicated[ int( rand() * scalar(@replicated) ) ];

        if ( defined $random_db ) {
            debug( 3,
                    "DBs being replicated are: "
                  . join( ', ', @replicated )
                  . "; random choice is $random_db" );
            ...
            while ( my $ref = $sth->fetchrow_hashref ) {
                $slave_data->{replication}->{table_status}->{$random_db}
                  ->{ $ref->{Name} } = $ref;
            }
        }
    }
    ...
    $port = $options->{'master-port'}
      || $slave_data->{replication}->{Master_Port};
    my $user = $options->{'master-user'}
      || $slave_data->{replication}->{Master_User};
    my $pass = $options->{'master-pass'} || $options->{'slave-pass'};
    debug( 1, "Connecting to master $host:$port as user $user" );
    $dbh = DBI->connect( "DBI:mysql:database=$schema;host=$host;port=$port", $user, $pass );
    ...
    $sql = "SHOW VARIABLES";
    $sth = $dbh->prepare($sql);
    debug( 2, "Getting master variables" );
    $res = $sth->execute;
    if (not defined ($res) || $res !~ /^\d+$/) {
      debug(1, "Failed to get master data");
    ...
    debug(4, Dumper($master_data));

    if ( defined $options->{'check-random-database'} ) {
        foreach my $database (
            keys( %{ $slave_data->{replication}->{table_status} } ) )
        {
            ...
            $res = $sth->execute;
            while ( my $ref = $sth->fetchrow_hashref ) {
                $master_data->{replication}->{table_status}->{$database}
                  ->{ $ref->{Name} } = $ref;
            }
        }
    }
...
# The two arguments, 'master' and 'slave' are references to the hashes 
# from the respective SHOW MASTER STATUS queries. Both must be present, or 
# we return nothing. We compare each, avoiding deficiencies in InnoDB tables, 
# looking only at the row counts. If the row_count difference exceeds our 
# limit, we add it to our message string.
    my %args = @_;
    ...
    foreach my $database ( keys %{ $args{slave} } ) {
        foreach my $table ( keys %{ $args{slave}->{$database} } ) {
            ...
            if (
                (
                    defined( $args{slave}->{$database}->{$table}->{Engine} )
                    && $args{slave}->{$database}->{$table}->{Engine} eq 'InnoDB'
                )
                || ( defined( $args{slave}->{$database}->{$table}->{Type} )
                    && $args{slave}->{$database}->{$table}->{Type} eq 'InnoDB' )
              )
            {

# We can't rely on InnoDB's row count from 'SHOW TABLE STATUS' since it is 
# an approximation. For MySQL 4.1.10 and below, we cant even get an 
# Update_time, since this is NULL! The only thing we /could/ do is 
# reconnect and do a SELECT COUNT(*) FROM TABLE, but we can't be bothered!
                debug( 5, "Skipping check on InnoDB table $database.$table" );
            }
            else {
                if (   defined( $args{slave}->{$database}->{$table}->{Rows} )
                    && defined( $args{master}->{$database}->{$table}->{Rows} ) )
                {
                    my $row_diff =
                      abs( $args{slave}->{$database}->{$table}->{Rows} -
                          $args{master}->{$database}->{$table}->{Rows} );
                    if (
                        abs($row_diff) >
                        $options->{'table-rows-diff-absolute-crit'} )
                    {
                        ...
                        abs($row_diff) >
                        $options->{'table-rows-diff-absolute-warn'} )
                    {
                        push @messages, "$database.$table $row_diff";
                    ...
                    debug( 2, "$database.$table has now row count on slave!" )
                      unless
                        defined( $args{slave}->{$database}->{$table}->{Rows} );
                    debug( 2, "$database.$table has now row count on master!" )
                      unless
                        defined( $args{master}->{$database}->{$table}->{Rows} );
                }
            }
...
sub compare_status {
    ...
    $master->{replication}->{'File_No'} = $1
      if ( $master->{replication}->{'File'} =~ /(\d+)$/ );
    $slave->{replication}->{'File_No'} = $1
      if ( $slave->{replication}->{'Relay_Master_Log_File'} =~ /(\d+)$/ );

    # Get the slave position it is executing, being careful of the
    # key name change in MySQL 4.1 (case change)
    $slave->{replication}->{'Position'} =
         $slave->{replication}->{'Exec_Master_Log_Pos'}
      || $slave->{replication}->{'Exec_Master_log_pos'};
    ...
    debug( 3,
          ...
          . $master->{replication}->{'Position'} );
    debug( 3,
            " Slave:  "
          . $slave->{replication}->{'Master_Log_File'} . ":"
          . $slave->{replication}->{'Position'} );

    my $diff =
      $master->{replication}->{'File_No'} -
      $slave->{replication}->{'File_No'} +
      (
        (
            $master->{replication}->{'Position'} -
              $slave->{replication}->{'Position'}
        ) / $slave->{max_binlog_size}
      );

    debug( 1, "diff: $diff " );

    # Compare the table status if we have them
    my ( $exit_level, $table_diff_message ) = compare_table_rows(
        master => $master->{replication}->{table_status},
        slave  => $slave->{replication}->{table_status}
    );
    ...
    if ( defined $slave->{'replication'}->{Seconds_Behind_Master} ) {
        if ( $slave->{'replication'}->{Seconds_Behind_Master} > 3600 ) {
            $time_diff =
                int( $slave->{'replication'}->{Seconds_Behind_Master} / 3600 )
              . "h "
              . ( $slave->{'replication'}->{Seconds_Behind_Master} % 3600 )
              . " secs";
        }
        else {
            $time_diff =
              $slave->{'replication'}->{Seconds_Behind_Master} . " secs";
        }
    }
    ...
    $state .= ", $time_diff"
      if defined( $slave->{'replication'}->{Seconds_Behind_Master} );
    $state .= ", "
      . ( $options->{'master'} || $slave->{replication}->{Master_Host} ) . ":"
      . ( $options->{'master-port'} || $slave->{replication}->{Master_Port} )
      . " ("
      . $master->{version} . ") -> "
      . $options->{slave} . ":"
      . $options->{'slave-port'} . " ("
...
get_status( $options->{'slave'}, $options->{'slave-port'}, $options->{'schema'} );
